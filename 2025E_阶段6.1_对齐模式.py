'''
实验名称：2025电赛E题 阶段6.1 对齐模式(延迟开激光) — 四态状态机(SCAN→ALIGN→AIM) + cv_lite加速
功能：在SCAN和AIM之间增加ALIGN过渡模式，矩形锁定但激光未识别时，
     发送靶心坐标给MSPM0/STM32控制云台引导激光到靶上。
     ★阶段6.1改进: ALIGN模式下先不开激光,等靶心被云台拉到画面中心(<2cm)后再开,
     确保光斑痕迹集中在靶心附近,满足题目光板痕迹距靶心最大距离<2cm。
...
# ===== 阶段6.1: 居中开激光阈值 =====
CENTER_THRESHOLD_CM = 2.0        # 靶心距画面中心<2cm才开激光(满足光板痕迹要求)
...
# ==================== 阶段6.1: 靶心居中判��� ====================
def is_target_centered(rect_cx, rect_cy, rect_w, rect_h):
    """判断靶心距画面中心是否<2cm
    利用已检测到的矩形宽高做像素→厘米换算。
    检测分辨率320×240, 中心(160,120)。
    返回True表示靶心已居中, 可以安全开激光。"""
    if rect_cx <= 0 or rect_w <= 0 or rect_h <= 0:
        return False

    # 像素→厘米换算系数
    px_per_cm_x = rect_w / RECT_WIDTH_CM    # 矩形宽25cm对应多少像素
    px_per_cm_y = rect_h / RECT_HEIGHT_CM   # 矩形高17.5cm对应多少像素

    # 距画面中心的像素偏差
    dx_px = abs(rect_cx - DETECT_W // 2)    # DETECT_W=320, 中心=160
    dy_px = abs(rect_cy - DETECT_H // 2)    # DETECT_H=240, 中心=120

    # 转为厘米
    if px_per_cm_x <= 0 or px_per_cm_y <= 0:
        return False
    dx_cm = dx_px / px_per_cm_x
    dy_cm = dy_px / px_per_cm_y

    return dx_cm < CENTER_THRESHOLD_CM and dy_cm < CENTER_THRESHOLD_CM
...
# ==================== 主程序 ====================
try:
    print("2025E 阶段6.1 四态状态机(SCAN→ALIGN→AIM, ALIGN居中后开激光) 启动")
    ...
    while True:
        ...
        # ===== 激光控制（按模式自动, 阶段6.1: ALIGN居中后才开激光） =====
        if not laser_force:
            if work_mode in (MODE_AIM, MODE_CIRCLE) and flag_detected:
                laser_on()     # AIM/CIRCLE: 已对准, 激光常开
            elif work_mode == MODE_ALIGN and flag_detected:
                # 阶段6.1: 只在靶心被STM32拉到居中(<2cm)后才开激光
                # 确保光斑痕迹集中在靶心附近, 避免ALIGN阶段激光在靶上划长线
                centered = is_target_centered(rect_cx, rect_cy, rect_w, rect_h)
                if centered:
                    laser_on()
                else:
                    laser_off()  # 等待STM32把靶子拉到画面中心
            else:
                laser_off()    # SCAN 时关闭
        ...
        if work_mode == MODE_ALIGN:
            if laser_pin.value():
                ...
                # 激光未开, 等STM32把靶子拉到居中
                dx_px = abs(rect_cx - DETECT_W // 2) if rect_cx > 0 else 999
                dy_px = abs(rect_cy - DETECT_H // 2) if rect_cy > 0 else 999
                detail = "center dx:{} dy:{}".format(dx_px, dy_px)
...
==================== 阶段6.1 vs 阶段6 核心变化 ====================
  ...
  激光控制策略:
    阶段6:  进入ALIGN立即开激光 → STM32拉靶心到中心时激光一直在靶上划
    阶段6.1: 进入ALIGN先关激光 → STM32把靶心拉到居中(<2cm) → 再开激光
             → 激光从靶心附近开始, 后续AIM只是微调, 光斑痕迹集中在2cm内

  新增函数 is_target_centered():
    利用矩形宽高做像素→厘米换算, 判断靶心距画面中心(160,120)是否<2cm.
    参数 RECT_WIDTH_CM=25.0, RECT_HEIGHT_CM=17.5 需与实际靶纸一致.

  新增配置 CENTER_THRESHOLD_CM = 2.0 (满足题目光板痕迹要求)

  工作流程(修改后):
    SCAN → [矩形锁定3帧] → ALIGN(激光OFF) → STM32 PID靶心→中心
    → [居中<2cm] → 激光ON → K230检测光斑3帧 → AIM

  STM32侧无需修改: 激光开关由K230 GPIO42控制, STM32只接收坐标做PID.
...
==================== 版本演进 ====================
  ...
  阶段6.1: 320×240, cv_lite,   OTSU, 四态  ~35fps (ALIGN延迟开激光,光斑痕迹<2cm)
'''
