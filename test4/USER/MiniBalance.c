...
/* Motor1_Current_Position / Motor2_Current_Position 在 motor.c 中定义 */
...
#define CENTER_X     320     /* K230画面中心X */
#define CENTER_Y     240     /* K230画面中心Y */
...
static void enter_find_initial(void)
{
    ...
    PID_Reset(&pid_azimuth);
    PID_Reset(&pid_pitch);
    /* X轴: center=0, amp=80°, 不增长, dir=+1 */
    scan_start(0, 800, 1, 0, 800);
    /* Y轴: 固定仰角355° = phys_pos2 = -50 (-5°) */
    phys_pos2 = PITCH_FIND_PHYS;
    motor2_send_phys(PITCH_FIND_PHYS);
    update_oled();
}
...
static void enter_find_secondary(void)
{
    ...
    /* X轴: center=当前位置, amp=20°起, 逐步扩大, max=179.9°(避免180°歧义) */
...
static void enter_align(void)
{
    ...
    /* Y轴同样需要同步phys_pos2 */
    cur_single = Motor2_Current_Position;
    phys_pos2 = (cur_single > 1800) ? (cur_single - 3600) : cur_single;
    if (phys_pos2 >  PHYS_LIMIT) phys_pos2 =  PHYS_LIMIT;
    if (phys_pos2 < -PHYS_LIMIT) phys_pos2 = -PHYS_LIMIT;

    mode = MODE_ALIGN;
    motor_set_mode(MOTOR_MODE_SINGLE_DIR);  /* 直通, 响应快 */
    motor_set_speed(SPEED_TRACK);
    motor2_set_mode(MOTOR_MODE_SINGLE_DIR);
    motor2_set_speed(SPEED_TRACK);
    PID_Reset(&pid_azimuth);
    PID_Reset(&pid_pitch);
    update_oled();
}
...
/* ==================== K230帧处理(模式相关) ====================
 * 双轴PID:
 *   ALIGN(无激光,拉靶心到中心): X: e=CENTER_X-d0, Y: e=d1-CENTER_Y
 *   AIM  (有激光,光斑趋靶心)  : X: e=d2-d0,         Y: e=d1-d3
 * 方向说明(硬件实测):
 *   X轴反向: phys_pos增=电机左转(非约定右转) -> e=center-pos
 *   Y轴反向: phys_pos2增=电机下转(非约定上仰) -> e=pos-center (与X符号相反)
 */
static void process_k230_frame(void)
{
    ...
    int d1 = k230_frame.data1;
    int d2 = k230_frame.data2;
    int d3 = k230_frame.data3;
    int e, e_pitch;
    float out, out_pitch;
    ...
    if (mode == MODE_ALIGN) {
        ...
        if (!has_rect) {
            ...
            /* 有靶无激光: 双轴PID把靶拉到画面中心(320,240)
             * X轴反向: D0>320(靶在右)->需右转->phys_pos减->e=320-D0<0
             * Y轴反向: D1>240(靶在下)->需下转->phys_pos2增->e=D1-240>0 */
            e       = CENTER_X - d0;
            e_pitch = d1 - CENTER_Y;
            if (e > -ERR_DEADBAND && e < ERR_DEADBAND) e = 0;
            if (e_pitch > -ERR_DEADBAND && e_pitch < ERR_DEADBAND) e_pitch = 0;
            out       = PID_Update(&pid_azimuth, (float)e);
            out_pitch = PID_Update(&pid_pitch,   (float)e_pitch);
            phys_pos  += (int)out;
            phys_pos2 += (int)out_pitch;
            if (phys_pos  >  PHYS_LIMIT) phys_pos  =  PHYS_LIMIT;
            if (phys_pos  < -PHYS_LIMIT) phys_pos  = -PHYS_LIMIT;
            if (phys_pos2 >  PHYS_LIMIT) phys_pos2 =  PHYS_LIMIT;
            if (phys_pos2 < -PHYS_LIMIT) phys_pos2 = -PHYS_LIMIT;
            motor_send_phys(phys_pos);
            motor2_send_phys(phys_pos2);
        }
        return;
    }
    ...
    if (mode == MODE_AIM) {
        ...
        if (!has_rect) {
            ...
            /* 有靶有激光: 双轴PID让光斑(D2,D3)趋近靶心(D0,D1)
             * X轴反向: D2>D0(光斑偏右)->需左转->phys_pos增->e=D2-D0>0
             * Y轴反向: D3>D1(光斑偏下)->需上转->phys_pos2减->e=D1-D3<0 */
            e       = d2 - d0;
            e_pitch = d1 - d3;
            if (e > -ERR_DEADBAND && e < ERR_DEADBAND) e = 0;
            if (e_pitch > -ERR_DEADBAND && e_pitch < ERR_DEADBAND) e_pitch = 0;
...
int main(void)
{
    ...
    PID_Init(&pid_pitch,   KP_Z, KI_Z, KD_Z, PID_MAX_OUT);
