...
void USART2_IRQHandler(void)
{
	if(USART_GetITStatus(USART2,USART_IT_IDLE)!=RESET)
	{
		...
		{
			/* 机械角度反馈(type=0x02): 区分ID=1/2 */
			if(usart2_receive_data[2]==0x02)
			{
				if(usart2_receive_data[1]==0x01)
				{
					Motor1_Current_Position = (usart2_receive_data[5]<<8) | usart2_receive_data[6];
				}
				else if(usart2_receive_data[1]==0x02)
				{
					Motor2_Current_Position = (usart2_receive_data[5]<<8) | usart2_receive_data[6];
				}
			}
		}
